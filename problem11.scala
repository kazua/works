...
object problem11 {
    ...
    def getUpSideMax(sqa : List[List[String]], dc : Int, acl : Int) : Int = sqa match {
        case a if a.size == 0 => acl
        case a => {
            val rm = a.last.sliding(dc).map(_.map(_.toInt).product).max
            if (rm > acl) getUpSideMax(a.init, dc, rm) else getUpSideMax(a.init, dc, acl)
        }
    }
    def changeUptoSideSqa(sqa : List[List[String]], rc : Int, acl : List[List[String]]) : List[List[String]] = rc match {
        case a if a == acl.size => acl.reverse
        case a => changeUptoSideSqa(sqa, rc, sqa.map(_.zipWithIndex).flatten.filter(_._2 == acl.size).map(_._1) :: acl)
    }
    def changeUptoRightSlantSqa(sqa : List[List[String]], rc : Int, acl : List[List[String]]) : List[List[String]] = rc match {
        case a if a == acl.size => acl.reverse
        case a => changeUptoRightSlantSqa(sqa, rc, sqa(acl.size).drop(acl.size) ++ sqa(acl.size).take(acl.size) :: acl)
    }
    def changeUptoLeftSlantSqa(sqa : List[List[String]], rc : Int, acl : List[List[String]]) : List[List[String]] = rc match {
        case a if a == acl.size => acl.reverse
        case a => changeUptoLeftSlantSqa(sqa, rc, sqa(acl.size).takeRight(acl.size) ++ sqa(acl.size).dropRight(acl.size) :: acl)
    }
    def main(args : Array[String]) {
        val dc = 4
        val rc = 20
        println(List(getUpSideMax(square, dc, 0), getUpSideMax(changeUptoSideSqa(square, 20, Nil), dc, 0), getUpSideMax(changeUptoSideSqa(changeUptoRightSlantSqa(square, 20, Nil), 20, Nil), dc, 0), getUpSideMax(changeUptoSideSqa(changeUptoLeftSlantSqa(square, 20, Nil), 20, Nil), dc, 0)).max)
    }
}