...
object porkerbattle {
    ...
    def dealCards(cd : List[(String, Int)], sn : Int, acl : List[List[(String, Int)]]) : List[List[(String, Int)]] = acl match {
        case a if a.head.size == sn && a.tail.head.size == sn => List(a.head.sorted, a.tail.head.sorted)
        case a => {
    ...
    def main(args : Array[String]) {
        println(judge(dealCards(cdl, 5, List(Nil, Nil))))
    }
}